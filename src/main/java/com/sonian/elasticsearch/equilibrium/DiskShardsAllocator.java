...
/**
 * The {@link DiskShardsAllocator} re-balances the nodes allocations
...
public class DiskShardsAllocator extends AbstractComponent implements ShardsAllocator {
    ...
    class ApplySettings implements NodeSettingsService.Listener {
        ...
        public void onRefreshSettings(Settings settings) {
            ...
            DiskShardsAllocator.this.threshold = threshold;
            DiskShardsAllocator.this.weightFunction = new WeightFunction(indexBalance, shardBalance, primaryBalance);
        }
    }
    ...
    @Inject
    public DiskShardsAllocator(Settings settings, NodeInfoHelper nodeInfoHelper, NodeSettingsService nodeSettingsService) {
        super(settings);
        ...
        logger.debug("DiskShardsAllocator compSettings {}", compSettings.getAsMap().toString());
    ...
    public boolean rebalance(RoutingAllocation allocation) {
        final Balancer balancer = new Balancer(logger, allocation, weightFunction, threshold, nodeInfoHelper,
                minimumAvailablePercentage, minimumSwapDifferencePercentage, minimumSwapShardRelativeDifferencePercentage);
    ...
    public boolean shardSwap(RoutingAllocation allocation) {
        final Balancer balancer = new Balancer(logger, allocation, weightFunction, threshold, nodeInfoHelper,
                minimumAvailablePercentage, minimumSwapDifferencePercentage, minimumSwapShardRelativeDifferencePercentage);
        return balancer.nodeInfoHelper.shardSwap(allocation, this.minimumSwapDifferencePercentage, this.minimumSwapShardRelativeDifferencePercentage);
    }

    public Balancer getBalancer(RoutingAllocation allocation) {
        return new Balancer(logger, allocation, weightFunction, threshold, nodeInfoHelper,
                minimumAvailablePercentage, minimumSwapDifferencePercentage, minimumSwapShardRelativeDifferencePercentage);
    }
    ...
    public static class Balancer {
        ...
        public Balancer(ESLogger logger, RoutingAllocation allocation, WeightFunction weight, float threshold,
                        NodeInfoHelper nodeInfoHelper, double minimumAvailablePercentage,
                        double minimumSwapDifferencePercentage, double minimumSwapShardRelativeDifferencePercentage) {
            this.logger = logger;
            this.allocation = allocation;
            this.weight = weight;
            this.threshold = threshold;
            this.nodeInfoHelper = nodeInfoHelper;
            this.minimumAvailablePercentage = minimumAvailablePercentage;
            this.minimumSwapDifferencePercentage = minimumSwapDifferencePercentage;
            this.minimumSwapShardRelativeDifferencePercentage = minimumSwapShardRelativeDifferencePercentage;
            for (RoutingNode node : allocation.routingNodes()) {
                nodes.put(node.nodeId(), new ModelNode(node.nodeId()));
            }
            metaData = allocation.routingNodes().metaData();
            logger.debug("Min settings - minimumAvailablePercentage {}, minimumSwapDifferencePercentage {}, minimumSwapShardRelativeDifferencePercentage {}",
                    this.minimumAvailablePercentage, this.minimumSwapDifferencePercentage, this.minimumSwapShardRelativeDifferencePercentage);
        }
        ...
        public boolean move(MutableShardRouting shard, RoutingNode node) {
            ...
            for (ModelNode currentNode : nodes) {
                ...
                if (decision.type() == Type.YES && this.nodeInfoHelper.enoughDiskForShard(target.nodeId(), this.minimumAvailablePercentage)) { // TODO maybe we can respect throttling here too?
                        ...
                        if (!node.containsShard(shard)) {
                            ...
                            if (currentWeight <= minWeight) {
                                ...
                                if (currentDecision.type() == Type.YES || currentDecision.type() == Type.THROTTLE) {
                                    ...
                                    if (!this.nodeInfoHelper.enoughDiskForShard(node.getNodeId(), this.minimumAvailablePercentage)) {
                                        logger.warn("Not enough disk for shard [{}] on node [{}]", shard, node.getNodeId());
                                        break NOUPDATE;
                                    }
        ...
        private boolean tryRelocateShard(ModelNode minNode, ModelNode maxNode, String idx, float minCost) {
            ...
            if (index != null) {
                ...
                for (MutableShardRouting shard : shards) {
                    if (shard.started() && this.nodeInfoHelper.enoughDiskForShard(node.nodeId(), this.minimumAvailablePercentage)) {
                ...
                if (candidate != null) {
                    ...
                    if (decision.type() == Type.YES/* && this.enoughDiskForShard(candidate, minNode, stats)*/) { /* only allocate on the cluster if we are not throttled */
                        if (logger.isTraceEnabled()) {
